...
public class ExportItemTask extends ExportFile implements IExportTask {
    ...
    public void storeBasicInfo() {
        for (ItemStack itemStack : this.items) {
            ...
            ItemTags.getTagGroup().getTagsFor(itemStack.getItem());
            map.add("TagList", IconRUtils.tagKeyList2Json(ItemTags.getTagGroup().getTagsFor(itemStack.getItem())));
            this.metas.add(map);
        }
    }
    ...
    public void storeDisplayName(boolean isEnglish) {
        ...
        for (int i = 0; i < amount; ++i) {
            ...
            if (!isEnglish) {
                ...
                if (group != null) {
                    map.addProperty("CreativeTabName", group.getTranslationKey().getString());
                }
            }
        }
    }
    ...
    public void storeImages() {
        try {
            ...
            for (int i = 0; i < amount; ++i) {
                ...
                large.writeFile(ExportFile.of("Images/" + this.modId + "/Items/" +
                        Registry.ITEM.getId(itemStack.getItem()).getPath() + "-128px.png", true));
                small.writeFile(ExportFile.of("Images/" + this.modId + "/Items/" +
